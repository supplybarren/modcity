...
#include "../examples/example_win32_directx11/maddox.h"
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    if (checkbox_animation.find(id) == checkbox_animation.end()) {
        checkbox_animation.insert({ id, 0.f });
        checkbox_animation_timer.insert({ id, 0.f });
    }

    float alpha = checkbox_animation.at(id);

    const float square_sz = GetFrameHeight();
    const ImVec2 pos = window->DC.CursorPos;
    const float pad = ImMax(1.0f, IM_FLOOR(square_sz / 6.0f));
    const ImRect total_bb(pos - ImVec2(5, 0), pos + ImVec2(square_sz + (pad + 110), label_size.y * 2.0f - 6));
    ItemSize(total_bb, style.FramePadding.y);
    ItemAdd(total_bb, id);
    ...
    if (*v) {
        if (checkbox_animation.at(id) < 1.f) {
            checkbox_animation_timer.at(id) += ImGui::GetIO().DeltaTime * 3;
            checkbox_animation_timer.at(id) = std::clamp<float>(checkbox_animation_timer.at(id), 0.f, 1.f);
            checkbox_animation.at(id) = easing::OutCubic(checkbox_animation_timer.at(id));
        }
    }
    else {
        if (checkbox_animation.at(id) > 0.f) {
            checkbox_animation_timer.at(id) -= ImGui::GetIO().DeltaTime * 3;
            checkbox_animation_timer.at(id) = std::clamp<float>(checkbox_animation_timer.at(id), 0.f, 1.f);
            checkbox_animation.at(id) = easing::InCubic(checkbox_animation_timer.at(id));
        }
    }

    const ImRect check_bb(pos, pos + ImVec2(20, 20));
    window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, IM_COL32(0, 140, 255, 10), 2.f);
    window->DrawList->AddRect(check_bb.Min, check_bb.Max, IM_COL32(0, 140, 255, 30 + alpha * 225), 2.f);
    ImGui::PushClipRect(check_bb.Min + ImVec2(1, 1), check_bb.Max - ImVec2(1, 1), true);
    if (alpha * 15 > 1)
        window->DrawList->AddCircleFilled(check_bb.Min + (check_bb.Max - check_bb.Min) / 2, alpha * 15, IM_COL32(0, 140, 255, 255), 24);
    ImGui::RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(4, 4), IM_COL32(255, 255, 255, alpha * 255), 12);
    ImGui::PopClipRect();

    ImVec2 label_pos(pos.x + 33, check_bb.Min.y + 1);
    window->DrawList->AddText(label_pos, ImGui::GetItemFlags() & ImGuiItemFlags_Disabled ? IM_COL32(255, 255, 255, 150) : IM_COL32(255, 255, 255, 150 + alpha * 105), label);
    IMGUI_TEST_ENGINE_ITEM_INFO(id, label, window->DC.ItemFlags | ImGuiItemStatusFlags_Checkable | (*v ? ImGuiItemStatusFlags_Checked : 0));

    ImVec2 cur_pos = ImGui::GetCursorPos();
    ImGui::SetCursorPosX(cur_pos.x + 20);
    return pressed;
}
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(0, 16), window->DC.CursorPos + ImVec2(ImGui::GetContentRegionAvail().x, label_size.y + 26));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(0, 15));
    const ImRect add_bb(total_bb.Min, total_bb.Max + ImVec2(0, 8));
    ItemSize(add_bb, style.FramePadding.y);
    if (!ItemAdd(add_bb, id, &frame_bb))
        return false;
    ...
    const ImU32 frame_col = IM_COL32(0, 140, 255, 10);
    window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, frame_col, 2.f);
    window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, IM_COL32(0, 140, 255, 30), 2.f);

    ImVec2 preview_pos = frame_bb.Min + ImVec2(7, 4);
    window->DrawList->AddText(preview_pos, ImGui::GetItemFlags() & ImGuiItemFlags_Disabled ? IM_COL32(255, 255, 255, 150) : IM_COL32(255, 255, 255, 255), preview_value);

    // Render shape
    const float value_x2 = ImMax(frame_bb.Min.x, frame_bb.Max.x - arrow_size);
    if (!(flags & ImGuiComboFlags_NoArrowButton))
    {
        const ImU32 m_col = ImGui::GetItemFlags() & ImGuiItemFlags_Disabled ? IM_COL32(49, 48, 53, 255) : IM_COL32(240, 240, 240, 255);
        window->DrawList->AddLine(ImVec2(frame_bb.Max.x - 17, frame_bb.Min.y + 9), ImVec2(frame_bb.Max.x - 7, frame_bb.Min.y + 9), IM_COL32(0, 140, 255, 50));
        window->DrawList->AddLine(ImVec2(frame_bb.Max.x - 17, frame_bb.Min.y + 13), ImVec2(frame_bb.Max.x - 7, frame_bb.Min.y + 13), IM_COL32(0, 140, 255, 50));
        window->DrawList->AddLine(ImVec2(frame_bb.Max.x - 17, frame_bb.Min.y + 17), ImVec2(frame_bb.Max.x - 7, frame_bb.Min.y + 17), IM_COL32(0, 140, 255, 50));
    }

    window->DrawList->AddText(ImVec2(frame_bb.Min.x, frame_bb.Min.y - 17), IM_COL32(255, 255, 255, 150), label);
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags)
{
    ...
    PushStyleColor(ImGuiCol_PopupBg, ImVec4(0.f, 0.54f, 1.f, 0.04f)); //0, 140, 255, 10
    PushStyleColor(ImGuiCol_Border, ImVec4(0 / 255.f, 140 / 255.f, 255 / 255.f, 30 / 255.f));
...
bool ImGui::Combo(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int popup_max_height_in_items)
{
    ...
    SetNextWindowSizeConstraints(ImVec2(0, 0), ImVec2(FLT_MAX, FLT_MAX));
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    std::string label_val(label);
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID(label);

    if (slider_animation.find(id) == slider_animation.end()) {
        slider_animation.insert({ id, 0.f });
        slider_animation_timer.insert({ id, 0.f });
    }

    float alpha = slider_animation.at(id);

    const ImVec2 pos = window->DC.CursorPos;
    int origx = ImGui::GetCursorPosX();
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(175, 7 + (label_size.y + style.FramePadding.y * 2.0f)));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max);

    /* add items to menu size*/
    ItemSize(total_bb, style.FramePadding.y);
    ...
    if (hovered || g.ActiveId == id) {
        if (slider_animation.at(id) < 1.f) {
            slider_animation_timer.at(id) += ImGui::GetIO().DeltaTime * 5;
            slider_animation_timer.at(id) = std::clamp<float>(slider_animation_timer.at(id), 0.f, 1.f);
            slider_animation.at(id) = easing::InSine(slider_animation_timer.at(id));
        }
    }
    else {
        if (slider_animation.at(id) > 0.f) {
            slider_animation_timer.at(id) -= ImGui::GetIO().DeltaTime * 5;
            slider_animation_timer.at(id) = std::clamp<float>(slider_animation_timer.at(id), 0.f, 1.f);
            slider_animation.at(id) = easing::InSine(slider_animation_timer.at(id));
        }
    ...
    // Slider behavior
    ImRect grab_bb;
    ImRect custom_drag_bb(frame_bb.Min - ImVec2(2, 0), frame_bb.Max + ImVec2(2, 0));
    const bool value_changed = SliderBehavior(custom_drag_bb, id, data_type, p_data, p_min, p_max, format, flags, &grab_bb);
    if (value_changed)
        MarkItemEdited(id);
    // Render grab

    window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(0, 20), frame_bb.Max - ImVec2(0, 2), IM_COL32(0, 140, 255, 10), 3.f);
    window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(0, 20), ImVec2(grab_bb.Max.x, frame_bb.Max.y - 2), IM_COL32(0, 140, 255, 255), 3.f);
    window->DrawList->AddRect(frame_bb.Min + ImVec2(0, 20), frame_bb.Max - ImVec2(0, 2), IM_COL32(0, 140, 255, 30), 3.f);
    window->DrawList->AddRectFilled(grab_bb.Max - ImVec2(10, 10), grab_bb.Max + ImVec2(0, 3), ImGui::GetItemFlags() & ImGuiItemFlags_Disabled ? IM_COL32(49, 48, 53, 255) : IM_COL32(255, 255, 255, 255), 3.f);
    //window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(0, 21 - alpha / 127.5), grab_bb.Max + ImVec2(-1, 6 - alpha / 127.5), IM_COL32(90, 79, 207, 255), 3.f - alpha / 85, ImDrawCornerFlags_All);
   // PopClipRect();
    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);
    ImVec2 textsize = CalcTextSize(value_buf);
    window->DrawList->AddText(ImVec2(frame_bb.Max.x - 2 - textsize.x, pos.y + style.FramePadding.y - 2), IM_COL32(220, 220, 220, 150 + alpha * 105), value_buf);
    ImVec2 label_pos = ImVec2(pos.x + 2, pos.y + style.FramePadding.y - 3);
    window->DrawList->AddText(label_pos, ImGui::GetItemFlags() & ImGuiItemFlags_Disabled ? IM_COL32(255, 255, 255, 150) : IM_COL32(255, 255, 255, 150 + alpha * 105), label);

    ImGui::SetCursorPos(ImVec2(ImGui::GetCursorPosX() + 20, ImGui::GetCursorPosY() + 10));
    return value_changed;
}
...
{
    return ColorEdit4(label, col, flags | ImGuiColorEditFlags_NoAlpha | ImGuiColorEditFlags_NoInputs);
}
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        ImVec2 n_pos = ImVec2(ImGui::GetCursorPos().x + ImGui::GetWindowPos().x + 30, ImGui::GetCursorPos().y + ImGui::GetWindowPos().y + 4);
        window->DC.CursorPos = n_pos;

        const ImVec4 col_v4(col[0], col[1], col[2], alpha ? col[3] : 1.0f);
        if (ColorButton("##ColorButton", col_v4, flags, ImVec2(40, 16)))
        {
        ...
        if (BeginPopup("picker"))
        {
            ...
            SetNextItemWidth(square_sz * 8.f); // Use 256 + bar sizes?
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    float sv_cursor_rad = value_changed_sv ? 3.0f : 6.0f;
    draw_list->AddCircleFilled(sv_cursor_pos, sv_cursor_rad, user_col32_striped_of_alpha, 4);
    draw_list->AddCircle(sv_cursor_pos, sv_cursor_rad + 1, col_black, 4);
    draw_list->AddCircle(sv_cursor_pos, sv_cursor_rad, col_white, 4);
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size)
{
    ...
    if (size.y == 0.0f)
        size.y = default_size;
    const ImRect bb(window->DC.CursorPos + ImVec2(20, -2), window->DC.CursorPos + size + ImVec2(2, 4));
    ...
    else
    {
        ...
        if (col_source.w < 1.0f)
            ...
            window->DrawList->AddRectFilled(bb_inner.Min, bb_inner.Max, GetColorU32(col_source), 2.f);
    }
    RenderNavHighlight(bb, id);
