...
int main(int, char**)
{
    ...
    ImFont* icon = io.Fonts->AddFontFromFileTTF("C:\\Users\\17854\\AppData\\Local\\Microsoft\\Windows\\Fonts\\icons.ttf", 18.f, &font_config_normal, io.Fonts->GetGlyphRangesCyrillic());
    ImFont* font_bold = io.Fonts->AddFontFromFileTTF("C:\\Users\\17854\\AppData\\Local\\Microsoft\\Windows\\Fonts\\MuseoSans.ttf", 17.f, &font_config_bold, io.Fonts->GetGlyphRangesCyrillic());
    ImFont* font = io.Fonts->AddFontFromFileTTF("C:\\Users\\17854\\AppData\\Local\\Microsoft\\Windows\\Fonts\\MuseoSans.ttf", 16.f, &font_config_normal, io.Fonts->GetGlyphRangesCyrillic());
    ...
    while (!done)
    {
        ...
        ImGui::Begin("hotbar window", &m_open, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoCollapse);
        ImVec2 pos = ImGui::GetWindowPos();
        ImVec2 size = ImGui::GetWindowSize();
        auto drawlist = ImGui::GetWindowDrawList();
        drawlist->AddRectFilled(pos, ImVec2(pos.x + size.x, pos.y + size.y / 2), IM_COL32(13, 21, 42, 255), 2.f, ImDrawFlags_RoundCornersTop);
        ...
        drawlist->AddText(ImVec2(pos.x + 10, pos.y + 10), IM_COL32(255, 255, 255, 255), "hotbar window");
        ImGui::End();
        ImGui::PopStyleVar(2);
        ...
        // 2. Show a simple window that we create ourselves. We use a Begin/End pair to created a named window.
        {
            ...
            if (selected_tab == NIGGA_AIMBOT) {
                ...
                ImGui::EndChild();
                ImGui::SameLine();
                ImGui::BeginChild("Secondary");
                ImGui::Checkbox("checkboxa", &checkbox_2);
                ImGui::ColorEdit3("coloredit", col);
                ImGui::EndChild();
            }

            if (selected_tab == NIGGA_VISUALS) {
                static bool another = false;
                static bool check = false;
                static bool iambool = false;
                std::vector<const char*> subtabs = {"Main", "Second", "Third", "Fourth", "Fifth"};
                static int selected_value = 1;
                maddox::gui::subtab("testing", &selected_value, subtabs);
                if (selected_value == 0) {
                    ImGui::BeginChild("visuals_child");
                    ImGui::Checkbox("Another", &another);
                    ImGui::Checkbox("checkboxfor", &check);
                    ImGui::Checkbox("you", &iambool);
                    ImGui::EndChild();
                }
            }
            ...
            if (maddox::gui::tab("b", "Visuals", m_sidebar_animation)) {
                selected_tab = NIGGA_VISUALS;
            }
