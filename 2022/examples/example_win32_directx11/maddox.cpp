...
bool maddox::gui::keybind(const char* id, int* key_index) {
    ...
    ImRect total_bb(pos, ImVec2(pos.x + 40, pos.y + 20));
    ...
    drawlist->AddRectFilled(ImVec2(pos.x - 4, pos.y - 2), ImVec2(pos.x + 42, pos.y + 20), IM_COL32(0, 140, 255, 10), 2.f);
    drawlist->AddRect(ImVec2(pos.x - 4, pos.y - 2), ImVec2(pos.x + 42, pos.y + 20), IM_COL32(0, 140, 255, 20 + animation * 200), 2.f);
...
float up_button = 0.f;
float up_button_anim = 0.f;
float down_button = 0.f;
float down_button_anim = 0.f;

bool maddox::gui::subtab(const char* title, int* selected, std::vector<const char*> options) {
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    ImGuiID hash = window->GetID(title);
    ImVec2 pos = ImGui::GetWindowPos();
    ImVec2 cur_pos = ImGui::GetCursorPos();
    ImVec2 size = ImGui::GetWindowSize();
    pos = ImVec2(pos.x + cur_pos.x + 15, pos.y + 20);
    auto drawlist = window->DrawList;
    
    drawlist->AddRectFilled(pos, ImVec2(pos.x + 30, pos.y + 30), IM_COL32(0, 140, 255, 10), 2.f);
    drawlist->AddRect(pos, ImVec2(pos.x + 30, pos.y + 30), IM_COL32(0, 140, 255, 20 + down_button_anim * 220), 2.f);
    ImVec2 points[] = { {pos.x + 20, pos.y + 10}, {pos.x + 10, pos.y + 15}, {pos.x + 20, pos.y + 20} };
    drawlist->AddConvexPolyFilled(points, 3, IM_COL32(0, 140, 255, 200 + down_button_anim * 55));

    drawlist->AddRectFilled(ImVec2(ImGui::GetWindowPos().x + size.x - 50, pos.y), ImVec2(ImGui::GetWindowPos().x + size.x - 20, pos.y + 30), IM_COL32(0, 140, 255, 10), 2.f);
    drawlist->AddRect(ImVec2(ImGui::GetWindowPos().x + size.x - 50, pos.y), ImVec2(ImGui::GetWindowPos().x + size.x - 20, pos.y + 30), IM_COL32(0, 140, 255, 20 + up_button_anim * 220), 2.f);
    ImVec2 points2[] = { {ImGui::GetWindowPos().x + size.x - 40, pos.y + 10}, {ImGui::GetWindowPos().x + size.x - 30, pos.y + 15}, {ImGui::GetWindowPos().x + size.x - 40, pos.y + 20} };
    drawlist->AddConvexPolyFilled(points2, 3, IM_COL32(0, 140, 255, 200 + up_button_anim * 55));

    ImRect back_button(pos, ImVec2(pos.x + 60, pos.y + 30));
    ImRect next_button(ImVec2(ImGui::GetWindowPos().x + size.x - 80, pos.y), ImVec2(ImGui::GetWindowPos().x + size.x - 20, pos.y + 30));

    if (ImGui::IsMouseHoveringRect(back_button.Min, back_button.Max) && ImGui::IsMouseClicked(0)) {
        down_button = 1.f;
        if (*selected - 1 >= 0)
            *selected -= 1;
    }

    if (ImGui::IsMouseHoveringRect(next_button.Min, next_button.Max) && ImGui::IsMouseClicked(0)) {
        up_button = 1.f;
        if (*selected + 1 <= options.size() - 1)
            *selected += 1;
    }

    if (down_button > 0.f) {
        down_button -= ImGui::GetIO().DeltaTime * 3;
        down_button = std::clamp(down_button, 0.f, 1.f);
        down_button_anim = easing::OutCubic(down_button);
    }

    if (up_button > 0.f) {
        up_button -= ImGui::GetIO().DeltaTime * 3;
        up_button = std::clamp(up_button, 0.f, 1.f);
        up_button_anim = easing::OutCubic(up_button);
    }

    ImVec2 textsize = ImGui::CalcTextSize(options.at(*selected));
    drawlist->AddText(ImVec2(ImGui::GetWindowPos().x + 75 + 217 - textsize.x / 2, pos.y + 6), IM_COL32(255, 255, 255, 255), options.at(*selected));
...
void maddox::setup() {
    if (ficon == nullptr) {
        ...
        ficon = io.Fonts->AddFontFromFileTTF("C:\\Users\\17854\\AppData\\Local\\Microsoft\\Windows\\Fonts\\icons.ttf", 30.f, &cfg_, io.Fonts->GetGlyphRangesCyrillic());
        largeFont = io.Fonts->AddFontFromFileTTF("C:\\Users\\17854\\AppData\\Local\\Microsoft\\Windows\\Fonts\\MuseoSans.ttf", 20.f, &cfg_, io.Fonts->GetGlyphRangesCyrillic());
        largeFont_nb = io.Fonts->AddFontFromFileTTF("C:\\Users\\17854\\AppData\\Local\\Microsoft\\Windows\\Fonts\\MuseoSans.ttf", 11, &cfg, io.Fonts->GetGlyphRangesCyrillic());
    }
}
